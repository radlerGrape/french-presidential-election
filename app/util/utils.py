...
def get_poll_results(link):
    ...
    _df_results["Result date"] = datetime.today().strftime("%Y-%m-%d")
...
def get_poll_statistics(link):
    ...
    _df_turnout["Result date"] = datetime.today().strftime("%Y-%m-%d")
...
def get_states_poll():
    ...
    for row in tqdm(df_departments_dict):
        ...
        with open(f"{data_path}/states_results.csv", mode="a") as f:
...
def get_overall_poll():
    ...
    with open(f"{data_path}/overall_results.csv", mode="a") as f:
